...
} from "@/app/redux/api/features/product/productApi";
...
const ShopDetails = () => {
  const [cyclecolor, setColor] = useState("");
  ...
  const newBycle = {
    ...
    name:bycle.name,
    price: bycle.price,
    category: bycle.category,
    image_url:bycle.image_url,
    
  }

  const cardAdd = () => {
    dispatch(addToCart({ ...newBycle, quantity: 1, color: cyclecolor }));
        ...
        <div className="flex justify-center items-center absolute top-0 left-0 w-full h-full bg-[#363535b4]">
          <div className=" space-y-4">
            ...
            <div className=" flex justify-center items-center text-white ">
              ...
              <Link className="hover:text-green-400" to="/shop">
                Shop /
              </Link>
        ...
        <div className=" p-8">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12  py-10 rounded-xl ">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-4">{bycle?.description}</p>
              ...
              <div className="text-sm text-gray-500 mt-6">
                ...
                <p>
                  Tags:{" "}
                  {bycle?.tags?.map((tag: any, index: number) => (
                    <span key={index}>
                      {tag}
                      {index < bycle.tags.length - 1 ? ", " : ""}
                    </span>
                  ))}
                </p>{" "}
                <p>Product ID: {bycle?._id}</p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto mt-10">
            <Tabs defaultValue="description">
              <TabsList className="flex  justify-start mb-5 bg-none shadow-none border-b">
                <TabsTrigger
                  value="description"
                  className="px-6 py-3 text-lg font-semibold border-r"
                >
                  Description
                </TabsTrigger>
                <TabsTrigger
        ...
        <div className=" ">
          <div className="mx-auto max-w-7xl px-4 lg:py-10 py-10 grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {bycles &&
              bycles?.data
                ?.slice(0, 3)
                ?.map((bike) => <ShopCard key={bike.id} bike={bike} />)}
          </div>
        </div>
      </div>
    </div>
